...
class WordPaginator(standard_paginator.Paginator):
    ...
    def create_from_blocks(
      ...
      """
        Create a paginator from a list of blocks.

        Args:
            client: A reference to the client
            content: The blocks to paginate
            blocks_per_page: The amount of blocks to have for each page
            timeout: A timeout to wait before closing the paginator
            allow_multi_user: Whether multiple users can use the buttons

        Returns:
            A paginator system
        """
      pages = [list(i) for i in zip(*[iter(content)] * blocks_per_page)]
      pages.append(content[content.index(pages[-1][-1])+1:])
      pages = ["\n".join(page) for page in pages]
      pages = [standard_paginator.Page(c, prefix="", suffix="") for c in pages]
